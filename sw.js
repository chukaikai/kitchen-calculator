const CACHE_NAME = 'kitchen-os-v310-item-master';
const ASSETS = [
  './',
  './index.html',
  './weekly.html',
...
function isKitchenPage(request) {
  ...
  return url.origin === self.location.origin &&
    (url.pathname.endsWith('/daily.html') || url.pathname.endsWith('/import.html'));
}

function injectMasterScript(html) {
  if (html.includes('item-master.js')) return html;
  return html.replace('</head>', '<script src="./item-master.js?v=310" defer></script></head>');
}
...
function patchDailyDelivery(html) {
  ...
  const replacement = `function applyDelivery(x){
    if(!x)return;
    const fixed=window.KOSItemMaster?window.KOSItemMaster.buildDeliveryIdMaps(x):x;
  ...
  return html.replace(/function applyDelivery\(x\)\{[\s\S]*?\}\nlet deliveryLabel=/, `${replacement}\nlet deliveryLabel=`);
}
